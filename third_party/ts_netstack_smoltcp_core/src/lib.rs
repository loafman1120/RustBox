...
impl Netstack {
    ...
    fn pump_blocked_commands(&mut self) {
        ...
        for _ in 0..self.blocked_commands.len() {
            ...
            // Async socket operations are cancelable by dropping their response
            // receiver. Retrying such a command after Close has removed its
            // socket handle would panic in smoltcp's SocketSet::get_mut.
            if cmd.resp.is_disconnected() {
                continue;
            }
            self.process_one_cmd(cmd);
        }
