//! XTLS-Vision padding wrapper adapted from MIT `meow-proxy` 0.17.0.
...
impl VisionConn {
    pub fn new(inner: VlessStream, user_uuid: [u8; UUID_LEN]) -> Self {
        Self {
            inner,
            user_uuid,
    ...
    fn filter_server_tls(&mut self, chunk: &[u8]) {
        let _ = self.read_tls_filter.observe(chunk);
    }
}
