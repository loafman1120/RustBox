...
use rustbox_host_api::{
    ...
    PacketDeviceConfig, PacketDeviceError, PacketDeviceLease, PacketDeviceProvider,
};
...
mod imp {
    ...
    pub async fn release_lease(lease: NetworkLease) -> Result<(), NetworkControlError> {
        if !lease.active {
            return Ok(());
        }
        let handle = Handle::new().map_err(error)?;
...
#[cfg(target_os = "macos")]
use imp::{apply_transaction, open_packet_device, release_lease};

#[cfg(not(target_os = "macos"))]
fn open_packet_device(_config: PacketDeviceConfig) -> Result<PacketDeviceLease, PacketDeviceError> {
    Err(PacketDeviceError::new(
        "macOS packet devices are unavailable on this target",
    ))
}
#[cfg(not(target_os = "macos"))]
async fn apply_transaction(
    ...
    Err(NetworkControlError::new(
        "macOS network control is unavailable on this target",
    ))
}
#[cfg(not(target_os = "macos"))]
async fn release_lease(_lease: NetworkLease) -> Result<(), NetworkControlError> {
    Err(NetworkControlError::new(
        "macOS network control is unavailable on this target",
    ))
}
